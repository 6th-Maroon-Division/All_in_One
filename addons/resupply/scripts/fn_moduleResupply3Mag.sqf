...
if (_activated) then {
	_this spawn {
		...
		{
			_box addMagazineCargoGlobal [primaryWeaponMagazine _x select 0, 3];
			_box addMagazineCargoGlobal [handgunMagazine _x select 0, 1];
			_box addMagazineCargoGlobal [primaryWeaponMagazine _x select 1, 2];
		} forEach allPlayers;
